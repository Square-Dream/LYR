...
def get_openai_client():
    ...
    try:
        ...
        # API 키 설정 방법 2: 직접 전달
        return OpenAI(api_key="your-api-key-here")  # 실제 API 키로 교체
    except Exception as e:
                ...
                messages=[
                    {
                        "role": "user",
                        "content": [
                            {"type": "text", "text": "이 웹툰 이미지를 분석해주세요. 캐릭터의 감정, 표정, 행동을 설명하고, 이미지의 분위기와 상황을 파악해주세요. 감정과 분위기를 나타내는 키워드를 5-10개 추출해주세요."},
...
def extract_keywords(content, content_type='webtoon', num_keywords=15):
    ...
    try:
        ...
        if content_type == 'webtoon':
            ...
            image_keywords = []
            if 'combined_image_path' in content and content['combined_image_path'] and os.path.exists(content['combined_image_path']):
                try:
                    # 이미지 크기 확인
                    img = Image.open(content['combined_image_path'])
                    if img.width > 100 and img.height > 100:  # 유효한 이미지인지 확인
                        image_keywords = analyze_image_content(content['combined_image_path'])
                        print(f"Keywords from image analysis: {image_keywords}")
                except Exception as img_error:
                    print(f"Error in image analysis: {img_error}")
            
            # 개별 이미지 분석 (선택적) - 최대 1개 이미지만 분석
            individual_image_keywords = []
            if not image_keywords and 'image_paths' in content and content['image_paths']:
                try:
                    # 첫 번째 이미지만 분석
                    if os.path.exists(content['image_paths'][0]):
                        img_keywords = analyze_image_content(content['image_paths'][0])
                        individual_image_keywords.extend(img_keywords)
                        print(f"Keywords from individual image: {individual_image_keywords}")
                except Exception as img_error:
                    print(f"Error in individual image analysis: {img_error}")
            
            # 이미지 키워드 합치기
            image_keywords.extend(individual_image_keywords)
        else:  # novel
